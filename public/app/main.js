...
function initNodes() {
  ...
  var bigArray = [];
  //   console.log(nodesAndEpochs);
  for (var i = 0; i < nodesAndEpochs.length; i++) {
    if (!nodesAndEpochs[i]) continue;
...
function render() {
  ...
  if (simulate) {
    ...
    nodeUniforms.positionTexture.value =
      simulator.positionUniforms.positions.value;
    labelUniforms.positionTexture.value =
      simulator.positionUniforms.positions.value;

    nodeUniforms.nodeAttribTexture.value =
      simulator.nodeAttribUniforms.nodeAttrib.value;
    edgeUniforms.nodeAttribTexture.value =
      simulator.nodeAttribUniforms.nodeAttrib.value;
    labelUniforms.nodeAttribTexture.value =
      simulator.nodeAttribUniforms.nodeAttrib.value;
